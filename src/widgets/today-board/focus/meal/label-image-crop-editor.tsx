...
import type { LabelCropSession } from "@/modules/nutrition/ocr/label-crop-session";

type DragMode = null | "move" | "n" | "s" | "e" | "w" | "ne" | "nw" | "se" | "sw";

/**
 * Interactive rotate + crop editor. Preview is canvas-drawn (not CSS-rotated)
 * so the crop rectangle stays aligned with displayed pixels.
 */
export function LabelImageCropEditor({
  imageUrl,
  sourceWidth,
  sourceHeight,
  initialRotation = 0,
  ...
  onConfirm: (session: LabelCropSession) => void;
  onRetake: () => void;
  onCancel: () => void;
}) {
  const baseId = useId();
  ...
  return (
    <div className="space-y-3">
      <p className="text-sm font-bold">
        Rotate and crop so the nutrition panel fills the box. This app does{" "}
        <span className="underline">not</span> auto-correct perspective — keep the label
        flat and shoot straight-on when you can.
      </p>
